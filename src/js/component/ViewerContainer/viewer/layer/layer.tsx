...
import { SampleLayers, SampleLayerKey, SampleOverlay } from "@src/js/type/sample_overlay"
...
import { viewer } from "@src/js/viewer_canvas"
...
export const Layer: React.FC<Props> = ({ viewerSize, layers, rotate, isCrossed, imageCenterInfo, originalRadius }) => {
    if (!layers) return <></>
    ...
    return <>
        {
            ...overlays.map((overlay, i) => {
                const pos = calcRelativePosition({ x: 0, y: 0 }, imageCenterInfo, viewerSize, originalRadius)
                return <Overlay
